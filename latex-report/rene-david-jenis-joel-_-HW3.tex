...
\end{equation}
To test the performance of the  controller, we run 3 different scenarios:
\begin{itemize}
\item \textbf{Scenario 1:} $\theta_R=90^\circ$, $\theta[0]=0^\circ$ (90 degree turn)
\begin{center}
  \includegraphics[width = 0.8\textwidth]{ss-err-90}
\end{center}
...
\item \textbf{Scenario 3:} $\theta_R=45^\circ$, $\theta[0]=0^\circ$ (45 degree turn)
\begin{center}
  \includegraphics[width = 0.8\textwidth]{ss-err-45}
\end{center}
For this case, the wheels also stop turning at around $1.85\,s$, and there is a \textbf{steady-state} error of $0.78^\circ$.
\end{itemize}
In general, can see that the controller \textbf{asymptotically stabilizes} the error in all three scenarios, as we don't see divergence over time for any of the scenarios. %TODO: Is the oscilating case also asymptotically stable? 
\section*{Task 7}
...
%   %\includegraphics[width = 0.8\textwidth]{ss-err-180}
% \end{center}
\item \textbf{Scenario 3: Diagonal Error}\\
Starting position: $(x_0,y_0) = (0,0),\, \theta=0^\circ$ Target: $(x_g,y_g) = (-1,1)$
\begin{center}
  \includegraphics[width = 0.8\textwidth]{task_8_ss-err_3}
\end{center}
As we can see, the robot moves only in the $x$ direction, as this is the direction it is facing. The controller successfully reduces the longitudinal error $d_0$ to zero.
\end{itemize}
\section*{Task 9}
To test both controllers together, we simulate an scenario where 
\begin{itemize}
\item \textbf{Scenario 1:} Starting position: $(x_0,y_0) = (0,0),\, \theta=0^\circ$ Target: $(x_g,y_g) = (-1,1)$
\begin{center}
\section*{Task 10}
