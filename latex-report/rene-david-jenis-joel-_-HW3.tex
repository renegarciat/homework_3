...
\end{itemize}
We can see that the controller \textbf{asymptotically stabilizes} the error in all three scenarios, and the convergence is \textbf{monotone} without oscillations or overshoot. The controller is also \textbf{robust to discretization/actuator limits} since it does not require aggressive corrections that could lead to saturation or instability.
\section*{Task 7}
