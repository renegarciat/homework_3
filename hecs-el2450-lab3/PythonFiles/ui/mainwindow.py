...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def printNparse_serial(self, msg: bytes):
        split_data = msg.split(b'\n')
        for mes in split_data:
            if mes.__len__() > 0:
                self.logfileserial.write(str(self.mocaptime) + ';' + mes.decode() + '\n')
                if mes[7:10] == b'set':
                    ctrl_split = mes.split(b' ')
                    u_l = ctrl_split[2][5:]
                    u_r = ctrl_split[3][6:]
                    self.logfilecontrol.write(str(self.mocaptime) + ';' + u_r.decode() + u_l.decode() + '\n')
            ...
            if pollResponseControl[0][1]==select.POLLOUT: #checks if a connection to the controller exists
                ...
                if self.forwardbutton.isDown(): 
                    ...
                    if self.logbutton.isChecked():
                        self.printNparse_serial(serialprint_msg)
                    return
    ...
    def readserial(self):
      ...
      for mes in messages:
        ...
        if self.logbutton.isChecked():
            # self.printNparse_serial(mes)
            split_data = mes.split(b'\n')
            self.logfileserial.write(str(self.mocaptime) + ';' + split_data[1] + '\n')
...
#      TODO: not yet implemented
    ...
    def on_loggingbutton_clicked(self):
        ...
        if logfilename != '':
            self.logfilelineedit.setText(logfilename[0])
    ...
    def on_autocontrolbutton_clicked(self, checked):
        ...
        if checked:
            ...
            # Related to Simulated 'Jump'
            if self.simulationCheckBox.isChecked() and (self.mocapActive and self.mocapconnection):
                self.mocapconnection.setstartpose(xs, ys, self.pose[2])
                
                # UI update  
                self.xposlabel.setText('{0:.3f}'.format(xs))
                self.yposlabel.setText('{0:.3f}'.format(ys))
                
                # Map animation update
                (xs_map, ys_map, rots_map) = self.get_mapposition(xs, ys, self.pose[2])
                self.animation.update(xs_map, ys_map, rots_map)

            serialprint_msg2 = self.wlinkconnection.transmit_state(xs, ys, self.pose[2])
    ...
    def on_logbutton_clicked(self, checked):
        ...
        if checked:
            ...
            try:
                ...
                filename_control = name[:i] + '_control' + name[i:]
                self.logfileserial = open(filename,'a')
                self.logfile.write('t; x; y; theta\n')
                self.logfileserial.write('Time; Serial.print output \n')
                self.logfilecontrol = open(filename_control,'a')
                self.logfilecontrol.write('time; right; left\n')
            except:
