...
class MocapSIM(object):
    ...
    def execute(self):
        ...
        base = 0.160
        radius = 0.033
        x = self.pose[0] + 0.5*radius*(control[0]+control[1])*math.cos(self.pose[2]*math.pi/180)*sample_time
        y = self.pose[1] + 0.5*radius*(control[0]+control[1])*math.sin(self.pose[2]*math.pi/180)*sample_time
        theta = self.pose[2] + (radius/base)*(control[1]-control[0])*sample_time
        if theta>=180:
           theta=theta-360
        elif theta<-180:
           theta=theta+360
        ...
        # Related to Simulated 'Jump'
        # Apply Offsets to calculate the "Virtual" Pose
        final_x = self.pose[0] + self.offset_x
        final_y = self.pose[1] + self.offset_y
        final_theta = self.pose[2] + self.offset_theta
        ...
        self.pose = [final_x, final_y, final_theta]
...
#        return valid
    ...
    # Related to Simulated 'Jump'
    def setstartpose(self, xs, ys, ts):
        # Get the latest raw data from the socket
        pose_cm, _ = self.bodyinfo()
        
        raw_x = pose_cm[0] / 100.0
        raw_y = pose_cm[1] / 100.0
        raw_theta = pose_cm[2]

        # Calculate offsets
        self.offset_x = xs - raw_x
        self.offset_y = ys - raw_y
        self.offset_theta = ts - raw_theta
