...
class corridor_animation(QMainWindow):
    ...
    def initUI(self):
        ...
        # Initial Robot Pos
        ## Convert mocap coordinate (0,0) to QT Coordinate, adopted from mainwindow.py -> get_mapposition()
        xoffset = -1.1
        yoffset = 2.4
        xscale = 164.214847759152
        yscale = 165.860400829302
        self.x = (-xoffset)*xscale
        self.y = (-yoffset)*yscale*-1.0
        ...
        self.nodes_pos = {node:(gridpos[0]*self.grid_size,gridpos[1]*self.grid_size) for node,gridpos in self.nodes.items()}
...
#        self.draw_robot(1, 1, 0)
    ...
    def draw_robot(self, x, y, rot, qp: QtGui.QPainter):
        ...
        qp.drawLine(x,y,int(x+self.cw/3*math.cos(rot)),int(y+self.cw/3*math.sin(rot)))
        return

    def update(self,x,y,rot):
        self.x = x
        self.y = y
        self.rot = rot if rot != None else self.rot
